...
or clone
\url{https://github.com/Hansiongs/sealedge} at tag \code{v0.5.1} and install
editable from the repository root.  A versioned archive of that tag is
deposited on Zenodo with DOI
\url{https://doi.org/10.5281/zenodo.21329428}.  Platform dependencies for
...
can auto-set a default for explore).  Large \code{data\_cache/} OHLCV files
are external to the manuscript bundle: reviewers pre-cache
\code{BTCUSDT}, \code{ETHUSDT}, and \code{SOLUSDT} 1h series with history
before \code{train\_start = 2021-07-01} (universe filters need volume
lookback and age).  The script does not fetch market data automatically.
