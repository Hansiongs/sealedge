...
\begin{tikzpicture}[
  font=\footnotesize\sffamily,
  snode/.style={draw, rounded corners=1.5pt, align=center,
    inner sep=4pt, minimum height=0.9cm, minimum width=2.15cm,
    fill=black!4},
  arr/.style={-{Latex[length=1.6mm]}, thick},
  fail/.style={draw, rounded corners=1.5pt, align=center,
    inner sep=3pt, minimum height=0.75cm, fill=black!8}
]
  \node[snode] (s1) {Seal\\record};
  \node[snode, right=0.4cm of s1] (s2) {Verify\\OK};
  \node[snode, right=0.4cm of s2] (s3) {Tamper\\on disk};
  \node[fail, right=0.4cm of s3] (s4) {Verify\\fails};
  \node[snode, below=0.65cm of s2] (s5) {Fresh seal\\+ break once};
  \node[fail, right=0.4cm of s5] (s6) {Second break\\rejected};
  ...
  \draw[arr] (s5) -- (s6);
\end{tikzpicture}
\caption{Synthetic Claim~1 micro-demo path
  (\code{scripts/reproduce\_seal\_demo.py}).  The demo uses a temporary
  seal file and fixed digest; it does not open registered-experiment
  holdouts or touch \code{data\_cache/}.}
\label{fig:seal}
\end{figure}
