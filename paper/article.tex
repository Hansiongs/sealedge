...
flows from a shell.  The replication script calls the explore surface only.
...
\code{docs/} tree (built with \pkg{mkdocs}), and the narrative methodology
...
callables.  Explore never opens the holdout:
\begin{Code}
...
# common explore keys (sealed path):
result["spa_p_value"]
result["spa_naive_p_value"]  # legacy null when both available
...
(Section~\ref{sec:compute}).  Metric keys above match the committed sample
used in Section~\ref{sec:cases}.  Large SPA $p$-values are valid outputs.

Holdout evaluation and commit-time diagnostics (including PSR on the
committed path) use an explicit second step:
\begin{Code}
from quant_lib import run_commit
# irreversible: re-verify HMAC inputs, then break the seal once
# commit = run_commit("vol_compression_v1", cache_dir="./data_cache")
# commit.psr, commit.final_equity  # commit/reporting surface
\end{Code}
Default manuscript replication does not execute \fct{run\_commit}, so
reviewers can re-run explore without consuming the holdout.  A mismatch on
re-hash at commit raises rather than silently scoring a tampered cache.
...
This section maps package statistical surfaces to concrete implementations in
\pkg{quant\_lib}.  Subsections~\ref{subsec:claim-seal}--\ref{subsec:claim-wfa}
state the four primary claims in depth: established method, formal
construction where it matters for reviewers, modules, defaults, and
...
\end{enumerate}
The cryptographic primitives are standard.  What is package-specific is
...
The eleven tools above group into five audit clusters.  Multi-test accounting (Subsections~\ref{sssec:mt-fdr}--\ref{sssec:mt-journal}) sits next to Claim~3 (Subsection~\ref{subsec:claim-spa}).  Selection-inflation control (\ref{sssec:sl-dsr}--\ref{sssec:sl-label}) sits next to Claim~2 (Subsection~\ref{subsec:claim-psr}).  Path-risk quantification (\ref{sssec:pr-bootstrap}--\ref{sssec:pr-ess}) pairs with Claim~4 (Subsection~\ref{subsec:claim-wfa}).  Audit instruments (\ref{sssec:au-hypo}--\ref{sssec:au-pf}) pre-date Claims~1 and~4 -- PF weighting feeds the same edge pipeline.  Cost accounting (\ref{sssec:co-costs}--\ref{sssec:co-facade}) is the realism floor under the SPA inputs used by Claim~3.
...
Platform dependencies for reproduction include \code{numpy}, \code{pandas},
\code{scipy}, \code{numba}, and \code{optuna} (see
\code{replication/README.md}).  Seal operations require
\code{QUANT\_LIB\_HMAC\_SECRET} (the reproduction script can auto-set a
default for explore).  Large \code{data\_cache/} OHLCV files are external to
the manuscript bundle: reviewers pre-cache \code{BTCUSDT}, \code{ETHUSDT},
and \code{SOLUSDT} 1h series with history before \code{train\_start =
2021-07-01} (universe filters need volume lookback and age).  The script does
not fetch market data automatically.
